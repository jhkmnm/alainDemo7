...
export class OrganizationUnitsTreeComponent extends AppComponentBase {
  private _allOrganizationUnits: OrganizationUnitListDto[] = [];
  ...
  arrToTreeNode(): void {
    this.loading = true;
    this._treeData = this._arrayService.arrToTreeNode(this._allOrganizationUnits, {
      idMapName: 'id',
  ...
  ouCodeArrToIdArr(codes: string[]) {
    // const filterOUs = _filter(this._allOrganizationUnits, (u: OrganizationUnitListDto) => {
